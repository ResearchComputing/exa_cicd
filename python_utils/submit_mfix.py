...
## Required args
parser.add_argument('--commit-hash', dest='commit_hash', type=str, help='Git commit hash on mfix-exa develop branch', required=True)
parser.add_argument('--results-dir', dest='results_dir', type=str, help='Base directory to permanently store results' required=True)
parser.add_argument('--working-dir', dest='working_dir', type=str, help='High performance directory to initially output results to' required=True)
parser.add_argument('--np-list', dest='np_list', type=list, help='Run mfix exa with each np in np_list' required=True)
parser.add_argument('--nodes', dest='nodes', type=int, help='Number of nodes to run on', required=True)
parser.add_argument('--time-limit', dest='time_limit', type='int', help='Max walltime in hours', required=True)
## Optional args
